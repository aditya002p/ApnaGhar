...
const RequestedProperties = () => {
  ...
  const requestedProperty = property.filter(
    (item) => item.agentEmail?.toLowerCase() === user?.email.toLowerCase()
  );

  const handleAccepted = (row) => {
    console.log(row);
    axiosSecure.patch(`/offeredAmount/accepted/${row._id}`).then((res) => {
  ...
  const handleReject = (row) => {
    console.log(row);
    axiosSecure.patch(`/offeredAmount/reject/${row._id}`).then((res) => {
      console.log(res.data);
      if (res.data.modifiedCount > 0) {
        refetch();
        Swal.fire({
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-center">
        <h2 className="text-5xl font-bold text-center mt-10 mb-5">
          REQUESTED PROPERTY
        </h2>
      </div>
      ...
      <div className="overflow-x-auto w-80 md:w-[710px] lg:w-full rounded-xl shadow-2xl mb-20">
        <table className="table">
          ...
          <tbody>
            {requestedProperty.map((row, index) => (
              <tr key={row._id}>
                <th>{index + 1}</th>
                ...
                {row.status === "Pending" && (
                  <td className="font-bold text-warning">{row.status}</td>
                )}
                {(row.status === "Accepted" || row.status === "Bought") && (
                  <td className="font-bold text-green-500">{row.status}</td>
                )}
                {row.status === "Rejected" && (
                  <td className="font-bold text-red-500">{row.status}</td>
                )}

                <td>
                  <button
                    disabled={row.status !== "Pending"}
                    onClick={() => handleAccepted(row)}
