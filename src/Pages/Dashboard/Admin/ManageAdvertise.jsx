...
const ManageAdvertise = () => {
  ...
  const handleAddAdvertise = (property) => {
    ...
    verifiedProperties.map((item) => {
      if (
        item.advertiseStatus &&
        item.advertiseStatus.toLowerCase() === "verified"
      ) {
        totalAdvertise++;
      }
    });
    ...
    if (totalAdvertise >= 6) {
      Swal.fire({
        ...
        icon: "error",
        title:
          "You have already exceed the maximum limit of advertise property",
    ...
    axiosSecure
      .patch(`/properties/verifiedAdvertise/${property._id}`)
      .then((res) => {
  ...
  const handleRemoveAdvertise = (property) => {
    ...
    axiosSecure
      .patch(`/properties/removedAdvertise/${property._id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data.modifiedCount > 0) {
          refetch();
    ...
    <div>
      ...
      <div className="container mx-auto px-5 ">
        <div className="flex items-center justify-center">
          <h2 className="text-5xl font-bold text-center mt-10 mb-5 uppercase">
            Advertise property
          </h2>
        </div>
            ...
            <tbody>
              {verifiedProperties.map((property, index) => (
                <tr key={property._id}>
                  ...
                  <th>
                    <button
                      disabled={property.advertiseStatus === "Verified"}
