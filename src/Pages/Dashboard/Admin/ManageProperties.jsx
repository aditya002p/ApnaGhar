...
const ManageProperties = () => {
  ...
  const handleVerify = (row) => {
    console.log(row);
    axiosSecure.patch(`/properties/verify/${row._id}`).then((res) => {
  ...
  const handleReject = (row) => {
    console.log(row);
    axiosSecure.patch(`/properties/reject/${row._id}`).then((res) => {
      console.log(res.data);
      if (res.data.modifiedCount > 0) {
        refetch();
        Swal.fire({
          ...
          <tbody>
            {property.map((row, index) => (
              <tr key={row._id}>
                ...
                <td>
                  {row.status === "Verified" && (
                    <p className="font-bold text-green-500">{row.status}</p>
                  )}
                  {row.status === "Rejected" && (
                    <p className="font-bold text-red-500">{row.status}</p>
                  )}
                  {row.status === "Pending" && (
                    <button
